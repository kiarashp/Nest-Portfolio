...
**`GET /products/:id/related`:** public; 404s under the same rule as `GET /products/:id` (missing or unpublished anchor product). Returns up to `?limit=` (default 4, max 20) other published products sharing the anchor's `productTypeId`, newest first, excluding the anchor itself — no fallback to other product types if too few same-type siblings exist.
