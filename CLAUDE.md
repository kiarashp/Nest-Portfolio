...
`ConfiguratorModule` (`src/configurator/`) is the ordering-code configurator — see **`CONFIGURATOR.md`** for the full design and 7-step implementation plan. It owns four entities (`ConfigurableProduct`, `SegmentDefinition`, `SegmentOption`, `ProductSegmentAssignment`) and imports `PaginationModule`, `UploadsModule`, and `AuditLogModule`. As of Step 2, it has one controller, `ConfiguratorDefinitionsController` — admin-only (`@Roles(UserRole.ADMIN)`) CRUD for the reusable `SegmentDefinition` library and its `SegmentOption` children. The controller has no base `@Controller()` prefix so its handlers can carry two different literal path families side by side (`/configurator-definitions/*` and `/configurator-options/*`), per `CONFIGURATOR.md` §5.1. Non-obvious rules enforced in the provider layer: a `SegmentOption.value` of `'0'` is always rejected (reserved zero-fill marker); options can only be added to a `SELECT` definition; a definition's `dataType` becomes immutable (409) once any `ProductSegmentAssignment` references it, and deleting an assigned definition/option is likewise blocked with a 409 — all unreachable via the API until Step 4 ships assignment creation, but implemented now per the design doc and exercised in `test/configurator/definitions.e2e-spec.ts` by seeding assignment rows directly through the repositories. `ConfigurableProduct` CRUD, assignments, and the public resolver are still pending (Steps 3–5).
