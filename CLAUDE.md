...
- `ConfigurableProduct` 1—N `ProductSegmentAssignment` N—1 `SegmentDefinition` 1—N `SegmentOption` — the configurator's join structure (fully separate from `Product`/`ProductType`, tables prefixed `configurator_`). `ProductSegmentAssignment.definitionId` is `onDelete: RESTRICT` (a definition in use cannot be deleted); `SegmentOption.definitionId` and `ProductSegmentAssignment.productId` are `onDelete: CASCADE`. `ConfigurableProduct` has `@DeleteDateColumn deletedAt` for soft-delete (mirrors `Product`); the other three configurator entities hard-delete. See `CONFIGURATOR.md` for the full entity/rules/API design — as of Step 1 only the entities and migration exist, no business logic.
