...
Nested guidance: `src/auth/CLAUDE.md` (RBAC, guards), `src/mail/CLAUDE.md`, `src/audit-log/CLAUDE.md`, `src/uploads/CLAUDE.md`, `src/products/CLAUDE.md`, `src/configurator/CLAUDE.md`, `test/CLAUDE.md`.
...
- **Gotcha — a running `start:dev` watch process races `migration:generate`:** if `pnpm run start:dev` is already running when you add/change an entity, Nest's watch mode recompiles and reconnects the moment the file is saved, and `DB_SYNC=true` auto-syncs the new schema into the dev DB before you get a chance to generate the migration — `migration:generate` then reports "No changes in database schema were found" because the dev DB already matches. Either stop the dev server before generating, or (if you don't want to interrupt a server you don't own) drop just the new tables/types via a throwaway `ts-node` script against `src/database/data-source.ts` immediately before running `migration:generate` — but the watch process will re-sync them right back within a second or two, so you may need to hand-verify the generated SQL against the dev DB's actual synced schema (e.g. `pg_dump --schema-only -t <table>`, `pg_enum`/`pg_type` for enum ordering) instead of relying on a clean generate.
...
`QUOTE_NOTIFY_EMAIL` (optional, falls back to `MAIL_FROM`) is the real inbox that receives configurator `POST /saved-configurations/:id/request-quote` notifications — same reasoning as `CONTACT_NOTIFICATION_EMAIL`. Maps to `mail.quoteNotifyEmail`, read by `SendQuoteRequestMailProvider`.
...
Module-per-domain layout: `users`, `posts`, `tags`, `auth` (with a nested `social` sub-feature for Google), `products`, `contact`, `configurator`, `admin`, plus cross-cutting `common`, `config`, `crypto`, and `audit-log` directories. `CryptoModule` (`src/crypto/`) is a standalone utility module that provides `HashingProvider` (abstract) and `BcryptProvider` (concrete); both `AuthModule` and `UsersModule` import it — this is how password hashing is shared without a circular dependency between those two modules. `AuditLogModule` (`src/audit-log/`) is imported by `UsersModule`, `PostsModule`, `TagsModule`, `ProductsModule`, `ContactModule`, and `ConfiguratorModule` — any module whose providers write audit records must import it. `ProductsModule` (`src/products/`) owns two entities (`ProductType`, `Product`) and two controllers (`ProductTypesController`, `ProductsController`); it imports `PaginationModule`, `UploadsModule`, and `AuditLogModule`.

`ConfiguratorModule` (`src/configurator/`) is the ordering-code configurator — see **`CONFIGURATOR.md`** for the full design and 7-step implementation plan, and **`src/configurator/CLAUDE.md`** for module structure, the resolver's behavior contract, and the non-obvious admin-side rules. It owns five entities (`ConfigurableProduct`, `SegmentDefinition`, `SegmentOption`, `ProductSegmentAssignment`, `SavedConfiguration`) and imports `PaginationModule`, `UploadsModule`, and `AuditLogModule`. As of Step 2, it has `ConfiguratorDefinitionsController` — admin-only (`@Roles(UserRole.ADMIN)`) CRUD for the reusable `SegmentDefinition` library and its `SegmentOption` children. The controller has no base `@Controller()` prefix so its handlers can carry two different literal path families side by side (`/configurator-definitions/*` and `/configurator-options/*`), per `CONFIGURATOR.md` §5.1. Non-obvious rules enforced in the provider layer: a `SegmentOption.value` of `'0'` is always rejected (reserved zero-fill marker); options can only be added to a `SELECT` definition; a definition's `dataType` becomes immutable (409) once any `ProductSegmentAssignment` references it, and deleting an assigned definition/option is likewise blocked with a 409. As of Step 3, it also has `ConfiguratorProductsController` (`/configurator-products`, a single path family so it uses a base `@Controller()` prefix unlike its sibling) — admin-only CRUD for `ConfigurableProduct` plus its single-slot image (`POST`/`DELETE /configurator-products/:id/image`), following the avatar-options pattern: `imageUrl`/`imagePublicId` are bare columns on the entity with no `UploadFile` row, uploaded/deleted directly via the injected `StorageProvider`. `imageUrl`/`imagePublicId` are deliberately absent from the create/update DTOs — only the image endpoints can set them, so a client can never hand-supply a Cloudinary `publicId` that a later delete call would pass to `StorageProvider.delete()`. Soft-deleting a `ConfigurableProduct` (`DELETE /configurator-products/:id`) deliberately keeps its Cloudinary image rather than purging it, unlike `Product`'s soft-delete — see `CONFIGURATOR.md` §2.1. As of Step 4, `ConfigurableProduct` has an inverse `assignments` relation to `ProductSegmentAssignment` (pure TypeORM metadata, no migration — the FK already existed via `productId`), and `GET /configurator-products/:id` returns the full ordered assignment tree (`assignments.definition.options`, ordered by `position`/`sortOrder`). `POST /configurator-products/:id/assignments` (default position: append) lives on `ConfiguratorProductsController`; a new `ConfiguratorAssignmentsController` (`/configurator-assignments`, base prefix) handles `PATCH`/`DELETE /configurator-assignments/:assignmentId` — not nested under a product id, since the assignment id alone locates its product. Gapless renumbering on insert/reorder/delete runs inside a `DataSource.createQueryRunner()` transaction (`src/configurator/providers/renumber-assignments.util.ts`, same manual-transaction pattern as `UserCreateManyProvider`) using a negate-then-finalize two-step shift — a naive single-statement `position = position ± 1` bulk update is **not** safe against the `(productId, position)` unique constraint, because PostgreSQL checks a unique constraint per row as it processes a multi-row `UPDATE`, not once at statement end, so a shifted row can momentarily collide with a not-yet-processed sibling in the same statement (negative positions can never collide with a real positive one, and negation is injective, so this two-step shift is safe regardless of row processing order). A `PATCH .../:assignmentId` position change is re-validated against the two direction-rule edges a uniform shift could break: the assignment's own condition (if any) must still point at a strictly lower position (400), and every other assignment targeting this one as its controller must still end up at a strictly higher position (409). Condition validation is split the same way `SegmentDefinition.constraints` is: `validate-assignment-condition.util.ts` (shape, dispatched on `operator`) and `validate-assignment-condition-rules.util.ts` (DB-dependent — controller exists at a strictly lower position in the same product, operator×dataType matrix per `CONFIGURATOR.md` §4.2, and a NUMBER-typed target definition must have `constraints.min >= 1`). As of Step 5, the public surface is live: `ConfiguratorsController` (`/configurators`, base prefix) exposes `GET /configurators/:slug` (curated form schema — product header + flattened segments keyed by `assignmentId`; no entity ids/slug/`imagePublicId`/`isPublished` leak) and `POST /configurators/:slug/resolve` (`@HttpCode(200)` — a computed result, not a created resource), both `@Auth(AuthType.None)` and neither audit-logged (reads/stateless). Both routes 404 on unknown, unpublished, or soft-deleted slugs via `FindOneConfigurableProductProvider.findOneBySlugPublishedOrFail`, which loads the same ordered assignment tree as the admin read. `ConfiguratorResolverService` (dependency-free, unit-tested with `new`) implements the §4.3 single-forward-pass algorithm: all validation errors are collected (never stops at the first), inactive segments zero-fill and are omitted from the summary, `code`/`summary` appear only when valid, and a per-segment `segments` state array is returned on every call. Two user-confirmed edge rules beyond the spec's wording: unknown-but-well-formed selection keys are silently ignored, and a dependent whose controller is active-but-errored zero-fills (cascade rule extended) — see `src/configurator/CLAUDE.md`. As of Step 6 (Phase 2), `SavedConfiguration` (`configurator_saved_configuration`) stores frozen snapshots (`productName`/`code`/`summary`/`selections`, never re-resolved — admin edits after saving have zero effect) owned by registered users: `POST /configurators/:slug/save` (on `ConfiguratorsController` — the one Bearer route there, bare `@ApiAuth()`, delegating to `SavedConfigurationsService` so `ConfiguratorsService` stays audit-free) re-resolves server-side, 400s with the resolver's error messages when invalid, and never trusts a client-composed code; `SavedConfigurationsController` (`/saved-configurations`, base prefix, any authenticated role) exposes the owner-scoped paginated list plus `GET`/`DELETE /:id`, where ownership is enforced by scoping the lookup to `{ id, userId }` so a foreign id 404s exactly like a missing one (deliberately not the posts-style 403). As of Step 7 (the final step), `POST /saved-configurations/:id/request-quote` stamps `quoteRequestedAt` and emails the site owner — `@HttpCode(HttpStatus.OK)` (200, not the `@Post()` default 201) since it mutates one column on an existing row rather than creating one, mirroring `PATCH /contact/:id`'s reasoning rather than `save`'s. `RequestQuoteSavedConfigurationProvider` checks `quoteRequestedAt` for the 409 idempotency guard **before** any mutation, save, audit log, or event emit, so a repeat call never re-triggers the notification email; a successful call sets `quoteRequestedAt`, audit-logs `AuditAction.UPDATE`/`'SavedConfiguration'`, then emits `AppEvents.QUOTE_REQUESTED` (payload interface in `src/common/events/app-events.ts`), handled by `QuoteEventsListener` (`src/configurator/listeners/`) which calls `MailService.sendQuoteRequestMail()`. `ConfiguratorModule` now imports `MailModule` (previously it didn't need mail) and registers `User` directly in its `TypeOrmModule.forFeature` — the same foreign-entity-in-a-cross-cutting-module pattern `AdminModule` uses — purely to resolve the requester's email/name for the notification, without importing all of `UsersModule`.

`AdminModule` (`src/admin/`) is a small cross-cutting module with a single route, `GET /admin/stats` (ADMIN only) — a dashboard aggregate (posts by status, products published/draft, product type count, user count, contact submission count). It owns no entities of its own; for this read-only cross-entity aggregation it registers `Post`, `Product`, `ProductType`, `User`, and `ContactSubmission` directly in its own `TypeOrmModule.forFeature` and injects each repository into `AdminStatsProvider` — the same pattern `AuditLogModule` uses for the foreign `User` entity, rather than importing `PostsModule`/`ProductsModule`/`UsersModule`/`ContactModule` just to reach their repositories.
...
- `Post` 1—N `UploadFile` (non-eager — loaded explicitly for the image picker, single-image delete, and cleanup on post deletion). `Post` also has a `@CreateDateColumn() createdAt: Date` used by the `startDate`/`endDate` filters on `GET /posts`. `Post.featuredImage` is nullable (`string | null`) so it can be explicitly cleared when the image it points at is deleted. `Post.excerpt` (nullable varchar(160)) is a short plain-text summary, client-settable on create/patch, meant for use as the SEO meta description / OG description. `Post.isFeatured` (boolean, default `false`) surfaces the post in a featured section and is filterable via `?isFeatured=` on `GET /posts`, `/posts/my`, and `/posts/admin`. `Post.publishedAt` (nullable timestamptz) is stamped automatically by `CreatePostProvider`/`UpdatePostProvider` the moment `status` becomes `PUBLISHED` — it is never settable via any DTO, unlike the similarly-named `publishOn` (a client-settable scheduled date that has no automatic transition logic anywhere in the codebase). `Post.contentHtml` (nullable text) is sanitized HTML rendered from `content` via `renderMarkdownToHtml` (`src/common/utils/render-markdown-to-html.util.ts`, `marked` + `sanitize-html` — shared with `Product.descriptionHtml`, see below) — re-rendered by `CreatePostProvider`/`UpdatePostProvider` whenever `content` is set/changed, never client-settable (absent from `CreatePostDto`/`PatchPostDto`, so `forbidNonWhitelisted` 400s a direct write attempt). A one-off backfill script, `src/database/scripts/backfill-post-content-html.ts`, re-renders `contentHtml` for posts that predate the column — not wired into any npm script, run manually. `Post.images` (nullable jsonb `string[]`) is a curated gallery — a client-chosen subset of the files uploaded via `POST /posts/:id/images` — client-settable on create/patch (send `null` to clear, omit to leave unchanged), mirroring `Product.images`. `DeletePostImageProvider` strips a deleted file's URL from `images` (in addition to clearing `featuredImage` if it pointed there) so the gallery never references a dead upload.
...
- `ContactSubmission` — standalone entity (`src/contact/entities/contact-submission.entity.ts`), no FK to any other table. Columns: `id`, `name`, `email`, `subject`, `message`, `createdAt`, `updatedAt`, `handled` (boolean, default `false`). Every submission is persisted permanently so the owner can review them even if an email notification is missed; `handled` lets an admin mark a submission as reviewed via `PATCH /contact/:id`.
- `AuditLog` — standalone entity (`src/audit-log/entities/audit-log.entity.ts`), no FK to any other table. Columns: `id`, `userId` (nullable int — null for self-service operations like registration), `action` (varchar 32), `entity` (varchar 64), `entityId` (int), `createdAt`. Written by providers after every successful write; never deleted. Also carries a transient (non-column) `user?: AuditLogUserSnapshot | null` field, attached by `FindAllAuditLogsProvider` for `GET /audit-logs` reads — see `src/audit-log/CLAUDE.md`.
- `ProductType` 1—N `Product` (non-eager — the inverse `products` relation is never loaded automatically; always query products directly). `ProductType` entity (`src/products/entities/product-type.entity.ts`) exports the `FilterableField` interface used in `filterableFields` (jsonb column). It also has a transient (non-column) `productCount` populated only by `GET /product-types` — the published-product count per type, for landing cards. `ProductType.imageUrl` (nullable varchar(1024)) is a real column carrying the image for the landing-page type card, client-settable on create/patch (send `null` to clear), and — like `Product`/`Post` images — backed by an `UploadFile` row (via the nullable `UploadFile.productTypeId` FK) so the Cloudinary asset is purged when the type is deleted or the image replaced; unlike products/posts, a type has only one image slot, so `POST /product-types/:id/image` uploads and sets `imageUrl` in a single combined call rather than the decoupled upload-then-PATCH flow. See `src/products/CLAUDE.md`.
- `Product` N—1 `ProductType` (eager — `productType` is always included in product responses). `Product` entity (`src/products/entities/product.entity.ts`) has `@DeleteDateColumn deletedAt` for soft-delete, a `specs` jsonb column (no DB index — TypeORM's `@Index` cannot express a `USING gin` index; add one via a raw-SQL migration if production scale needs it), and a B-tree `@Index(['productTypeId'])` at class level for type-based filtering. Soft-deleted products are automatically excluded from all TypeORM queries unless `withDeleted: true` is passed. `Product.isFeatured` (boolean, default `false`) mirrors `isPublished`'s shape and surfaces the product in a featured section; it is filterable via `?isFeatured=` on both the public and admin product routes (unlike `isPublished`, which is admin-route-only) and adds a `featured` value to `GET /products`' `sortBy` param (isFeatured first, then the `order`-directed tiebreak). `Product.related` is a transient (non-column) field, populated only by `GET /products/slug/:slug?includeRelated=N`. `Product.descriptionHtml` (nullable text) mirrors `Post.contentHtml` — sanitized HTML rendered from `description` via the same shared `renderMarkdownToHtml` util, re-rendered by `CreateProductProvider`/`UpdateProductProvider` whenever `description` is set/changed, never client-settable. A one-off backfill script, `src/database/scripts/backfill-product-description-html.ts`, re-renders `descriptionHtml` for products that predate the column — not wired into any npm script, run manually. See `src/products/CLAUDE.md`.
- `ConfigurableProduct` 1—N `ProductSegmentAssignment` N—1 `SegmentDefinition` 1—N `SegmentOption` — the configurator's join structure (fully separate from `Product`/`ProductType`, tables prefixed `configurator_`). `ProductSegmentAssignment.definitionId` is `onDelete: RESTRICT` (a definition in use cannot be deleted); `SegmentOption.definitionId` and `ProductSegmentAssignment.productId` are `onDelete: CASCADE`. `ConfigurableProduct` has `@DeleteDateColumn deletedAt` for soft-delete (mirrors `Product`); the other configurator entities hard-delete. `SavedConfiguration` (Phase 2) additionally references `User` (`userId`, non-eager, `onDelete: CASCADE` — no inverse relation on `User`, matching `UploadFile`) and `ConfigurableProduct` (nullable `productId`, `onDelete: SET NULL` — the snapshot survives a hard product delete; a soft-delete doesn't touch it). See `CONFIGURATOR.md` for the full entity/rules/API design.
...
**Sign-in `errorCode`:** `POST /auth/sign-in` returns a plain 401 for wrong credentials and for a Google-only account signing in locally, but the unverified-email branch additionally carries a machine-readable `errorCode: 'EMAIL_NOT_VERIFIED'` in the response body (`SignInProvider` throws `new UnauthorizedException({ message, errorCode })` — Nest passes an object exception payload through largely unchanged, merging in `statusCode`/`error`) so the frontend can distinguish "wrong password" from "needs verification" without string-matching the message. No other exception in the app carries an `errorCode` — this is a targeted fix for this one ambiguity, not a general app-wide error-code convention.
...
- `products`: `/slug/:slug`, `/sku/:sku`, and `/admin` before `/:id`
...
**Draft visibility (Products):** public routes hardcode `isPublished = true` in `FindAllProductsProvider` / `FindOneProductProvider`. `GET /products/admin` bypasses it (ADMIN only). `GET /products/:id/admin` (ADMIN only) is the single-product counterpart used by the admin edit form: it fetches one product by ID regardless of publish status, since `GET /products/:id` cannot return a draft for editing — mirrors `GET /posts/:id/admin` but with no ownership check, since product writes are already ADMIN-only.

**Self-targeting guard on user admin mutations:** `DELETE /users/:id`, `PATCH /users/:id/role`, and `PATCH /users/:id/verify-email` each throw `ForbiddenException` (403) when the target `id` equals the acting admin's own id — implemented as the first check in `RemoveOneByIdProvider.removeUserById`, `ChangeUserRoleProvider.changeUserRole`, and `SetEmailVerifiedProvider.setEmailVerified` respectively, before any DB read. This prevents an admin from deleting themselves, demoting themselves, or unverifying their own email — any of which could otherwise strand the system with no usable admin account. The generic `PATCH /users/:id` (name/email/password only) is deliberately **not** guarded — it cannot change role, delete, or unverify, so there is no lockout risk.

**`GET /products/:id/related`:** public; 404s under the same rule as `GET /products/:id` (missing or unpublished anchor product). Returns up to `?limit=` (default 4, max 20) other published products sharing the anchor's `productTypeId`, newest first, excluding the anchor itself — no fallback to other product types if too few same-type siblings exist. `GET /products/slug/:slug?includeRelated=N` (1-20, via `GetProductBySlugDto`) embeds the same related list inline as `Product.related` (a transient field, mirroring `ProductType.productCount`) so a slug-only product detail page can fetch the anchor and its related products in one request instead of two sequential ones — `ProductsService.findBySlug` composes `FindOneProductProvider.findOneBySlugOrFail` and `FindRelatedProductsProvider.findRelated`. `related` stays `undefined` (and is therefore omitted from the JSON response) unless `includeRelated` is sent, so every existing caller of the slug route sees the exact same shape as before.

**`GET /products/sku/:sku`:** public; an exact structural mirror of `GET /products/slug/:slug`, including `?includeRelated=N` support via the same `GetProductBySlugDto` and the same `isPublished`-only 404 rule (`FindOneProductProvider.findOneBySkuOrFail`, `ProductsService.findBySku`). Lets a frontend "scan/type a code" UI resolve `Product.sku` (unique, nullable, no format constraint — kept backend-agnostic, formatting is a frontend concern) straight to a product without a slug in hand.

**`GET /posts/:id/related`:** public; 404s under the same rule as `GET /posts/:id` (missing, draft, scheduled, or review anchor post — resolved via `FindOnePostProvider.findOnePublishedByIdOrFail`). Returns up to `?limit=` (default 4, max 20) other published posts sharing at least one tag with the anchor, newest first, excluding the anchor itself, via `FindRelatedPostsProvider`. If the anchor has no tags, the endpoint returns `[]` immediately — there is no fallback to unrelated posts, mirroring the products endpoint's no-fallback behavior. The tag-overlap check subqueries the `post_tags_tag` join table (not a condition on the `tags` join alias) for the same reason `FindAllPostsProvider`'s `tagIds` filter does — see the TypeORM gotchas section below.

**`GET /tags/:id`:** public, same access as `GET /tags` (no `@Roles()`). 404 if the tag doesn't exist. Backed by `FindOneTagProvider.findOneByIdOrFail`.

**Contact inbox (`GET /contact`, `GET /contact/:id`, `PATCH /contact/:id`):** all three ADMIN only (`POST /contact` stays public). `GET /contact` is paginated (`GetContactSubmissionsDto`, built on `FindAllContactSubmissionsProvider`) and filterable by `?handled=` (boolean) and a `?startDate=`/`?endDate=` range on `createdAt`; it uses `paginateQueryBuilder` (not the simple `where`-based `paginateQuery`) purely for guaranteed `ORDER BY createdAt DESC, id DESC` ordering, not because of joins. `GET /contact/:id` is a plain 404-on-missing single read. `PATCH /contact/:id` (`PatchContactSubmissionDto { handled: boolean }`) toggles the `handled` flag and writes an audit log entry (`AuditAction.UPDATE`, entity `'ContactSubmission'`) — unlike `UpdateTagProvider`, which does not audit-log its update. **`handled` boolean query-param gotcha:** the global `ValidationPipe`'s `enableImplicitConversion` coerces an unannotated `boolean`-typed property using `Boolean(value)` *before* any `@Transform` runs, so a raw query string `'false'` — being a non-empty string — is implicitly converted to `true` first, and a custom `@Transform` receives that already-wrong boolean instead of the original string. Adding `@Type(() => String)` above the `@Transform` on `GetContactSubmissionsDto.handled` stops the implicit conversion so the `@Transform` sees the real string and can correctly map `'true'`/`'false'` to real booleans. Apply the same `@Type(() => String)` guard to any future boolean query-param filter.

**`GET /admin/stats`:** ADMIN only. Returns one aggregate object: posts grouped by status (`draft`/`review`/`scheduled`/`published`, via a single grouped `COUNT(*) ... GROUP BY post.status` query, same idiom as `FindAllProductTypesProvider`'s per-type product count, with missing statuses defaulted to `0`), products split `published`/`draft`/`total` (two `repository.count({ where: { isPublished } })` calls — soft-deleted products are excluded automatically since `deletedAt` is a `@DeleteDateColumn`), plus plain counts of product types, users, and contact submissions. See the `AdminModule` note above for why the provider registers foreign entities directly instead of importing their owning modules.

**`PatchPostDto.featuredImage` clear semantics:** `PATCH /posts/:id` distinguishes "field omitted" from "explicitly cleared." Omitting `featuredImage` leaves the current value unchanged; sending `featuredImage: null` clears it — `UpdatePostProvider` checks `patchPostDto.featuredImage !== undefined` before assigning, not `??`, since `??` would silently discard an explicit `null` and keep the old value. Sending `featuredImage: ''` still 400s (`@IsUrl()` rejects empty string — `null` is the only valid clear signal). `CreatePostDto.featuredImage` stays `string | undefined` (nothing to clear on create); `PatchPostDto` (`src/posts/dto/update-post.dto.ts`) redeclares the field as `string | null` with its own `@IsUrl()`/`@MaxLength()`/`@IsOptional()` stack, so the "send null to clear" Swagger description only appears on the patch endpoint's OpenAPI schema, not on `POST /posts`. It cannot do this by extending `PartialType(CreatePostDto)` directly and overriding the property — TypeScript's override check rejects widening an inherited property's type (`string | undefined` → `string | null | undefined` fails with TS2416, caught by `nest build`/`ts-node` but not by ESLint). The fix is an intermediate class, `PatchPostDtoBase = PartialType(OmitType(CreatePostDto, ['featuredImage']))`, which `PatchPostDto` extends — omitting the field from the base first means there is nothing to override. Reuse this `OmitType`-then-`PartialType` pattern for any future patch DTO field that needs a wider type than its create-DTO counterpart. `Post.publishedAt` is never client-settable — sending it in any `POST`/`PATCH` body 400s (not a declared DTO field, rejected by `forbidNonWhitelisted`) — which is the key difference from the similarly-named `publishOn`, a client-settable scheduled date. `ProductType.imageUrl` needed no such workaround: it's declared as `string | null` directly on `CreateProductTypeDto` from the start (create and patch want the identical type there), so `UpdateProductTypeDto = PartialType(CreateProductTypeDto)` picks it up with no `OmitType` gymnastics.
...
**`GetPostsDto` query params** (shared by `GET /posts`, `/posts/my`, and `/posts/admin`): `limit`/`page`; `startDate`/`endDate` (Between/MoreThanOrEqual/LessThanOrEqual on `Post.createdAt` — `GET /posts` and `/posts/admin`, not `/posts/my`); `status` (`/posts/my` and `/posts/admin` — `GET /posts` hardcodes PUBLISHED so `status` is ignored there); `tagIds[]` (OR logic, `GET /posts` only — Transform converts string scalar to `number[]`); `authorId` (`GET /posts` and `/posts/admin`, not `/posts/my` which already scopes to the caller); `q` (1–100 chars, case-insensitive ILIKE on title+content, all three routes — composes as `(title OR content) AND (tag1 OR tag2)`); `sortBy` (`createdAt | title`, default `createdAt`) and `order` (`asc | desc`, default `desc`), both `@IsIn`-validated, all three routes — mirrors `GetAuditLogsDto`'s sort pattern below; `isFeatured` (boolean, same `@Type(() => String)` + `@Transform` guard as `GetContactSubmissionsDto.handled` below — works on all three routes, unlike `status`/`authorId` it is not restricted to `/posts/my`/`/posts/admin`, but `FindMyPostsProvider` deliberately does not wire it in, consistent with that provider's existing minimal filter set). `FindAllPostsProvider` (`findAll`/`findAllAdmin`) and `FindMyPostsProvider` build a `SelectQueryBuilder` (not the simple `where`/`paginateQuery` path) so the caller-controlled `ORDER BY` can be applied, with an `id` tiebreaker via the shared `apply-post-sort.util.ts` helper; both `leftJoinAndSelect` the eager `author`/`tags` relations since QueryBuilder does not auto-load them. The `tagIds` filter uses a subquery on the `post_tags_tag` join table rather than a condition on the `tags` join alias, so filtering by tag never restricts which tags are returned in a matched post's `tags` array.

**`GetProductsDto` query params:** `limit`/`page`; `productTypeId` (`@Type(() => Number)`) **or** `typeSlug` (matches the joined `productType.slug`; `productTypeId` wins if both sent); `q` (1–100 chars, ILIKE on name+shortDescription); `sortBy` (`createdAt` default / `name` / `featured`) and `order` (`asc` / `desc`, default `desc`), both `@IsIn`-validated — mirrors the `sortBy`+`order` shape used by `GetPostsDto`/`GetUsersDto`/`GetAuditLogsDto` (this is the one list endpoint that used to have its own fused `sort` enum; it was harmonized to this shape as a breaking API change, with no dual-support shim for the old `sort=newest|oldest|name|featured` param). `sortBy=featured` always sorts `isFeatured DESC` first — `order` only controls the direction of the `createdAt` tiebreak (and of `createdAt`/`name` themselves for the other `sortBy` values); see `FindAllProductsProvider.applySort`. `isPublished` (boolean, `GET /products/admin` only — the public `GET /products` route always hardcodes `isPublished = true` and ignores this field even if sent; uses the same `@Type(() => String)` + `@Transform` guard as `GetContactSubmissionsDto.handled` below, since a raw `'false'` query string is otherwise coerced to `true` by the global `ValidationPipe` before any custom transform runs); `isFeatured` (boolean, same guard — unlike `isPublished`, available on **both** the public and admin routes, since featured is a merchandising flag rather than a draft/publish gate); `specs` (bracket-nested params like `specs[sheathMaterial]=Inconel 600&specs[tempRange][min]=1000` parsed into a `Record` object — enum/string facets match exactly, number facets take a scalar or `[min]`/`[max]` range; requires a type context and is validated against the type's `filterableFields`. Needs the `extended` query parser — see `app.create.ts`). `FindAllProductsProvider` builds a `SelectQueryBuilder` (not a simple `where`) because spec filters need jsonb access — see `src/products/CLAUDE.md`. The provider `leftJoinAndSelect`s `productType` because QueryBuilder does not auto-load eager relations.

**`GetUsersDto` query params (`GET /users`, ADMIN only):** `limit`/`page`; `q` (1–100 chars, case-insensitive ILIKE across `firstName`, `lastName`, and `email`); `role` (`@IsEnum(UserRole)`, exact match); `isEmailVerified` (boolean, same `@Type(() => String)` + `@Transform` guard as `GetContactSubmissionsDto.handled`); `sortBy` (`id | firstName | lastName | email | role`, default `id` — `User` has no `createdAt` column, so `id` stands in for registration order) and `order` (`asc | desc`, default `asc`), both `@IsIn`-validated. `FindAllUsersProvider` builds a `SelectQueryBuilder` (mirroring `FindAllProductsProvider`/`FindAllPostsProvider`) instead of the simple `where`-based `paginateQuery` it previously used, so the caller-controlled `ORDER BY` can be applied, with an `id` tiebreaker when sorting by another column.

**`GetAuditLogsDto` query params (`GET /audit-logs`, ADMIN only):** `limit`/`page`; `entity`/`action` (exact-match filters); `sortBy` (`id | action | entity | entityId | userId | createdAt`, default `createdAt`) and `order` (`asc | desc`, default `desc`), both `@IsIn`-validated (invalid values 400 via the global `ValidationPipe`). `FindAllAuditLogsProvider` builds a `SelectQueryBuilder` (not `paginateQuery`) so the caller-controlled `ORDER BY` can be applied, with an `id` tiebreaker for stable pagination. Each row's `user` snapshot (see the `AuditLog` entity note above) is attached afterward via a single batch `User` lookup — see `src/audit-log/CLAUDE.md` for the full null/deleted semantics.
...
| `contact.submitted` | `SubmitContactProvider` | `ContactEventsListener` | Emails contact notification to site owner |
| `quote.requested` | `RequestQuoteSavedConfigurationProvider` | `QuoteEventsListener` | Emails quote-request notification to site owner |
...
- **`typeorm@1.0.0` also removed the string-array `select` syntax** (`select: ['id', 'firstName']`) on `find()`/`findOne()` calls — use object syntax instead (`select: { id: true, firstName: true }`), or it throws `TypeORMError` at request time, not at compile time (see `FindAllAuditLogsProvider`'s batch `User` lookup).
...
- **Filtering a to-many relation on the same alias used for `leftJoinAndSelect` also restricts what gets selected** — e.g. adding a `tags.id = :id` condition on the alias from `leftJoinAndSelect('post.tags', 'tags')` would return only the matching tag(s) in `post.tags`, not the post's full tag list. When a filter needs to narrow the result set without narrowing what a to-many relation selects, filter with a subquery against the join table instead (see `FindAllPostsProvider`'s `tagIds` filter, which subqueries `post_tags_tag`).
...
- **New dependency must support CommonJS require():** this app compiles to CommonJS (`nest build`, `tsconfig.typeorm.json`) and `pnpm run test` runs plain Jest with no ESM loader flag (unlike `test:e2e`, which sets `NODE_OPTIONS=--experimental-vm-modules` specifically for the `file-type` package — see the Uploads section). A dependency whose `package.json` `exports` map has no `require`/`default` condition pointing at a CJS build (ESM-only, e.g. `marked@16+`) fails with `SyntaxError: Unexpected token 'export'` the moment it's `require()`d, in both `pnpm run test` and the compiled runtime. Check a candidate package's `exports` field before adding it; pin to the latest version that still ships a CJS build if the alternative is a rewrite to dynamic `import()` everywhere it's used (`marked` is pinned to `^15.0.12` for exactly this reason — `sanitize-html` has no such issue, it's CJS-native).
