...
- Threaded `activeUserId` parameter — Users, Tags, Products, AvatarOptions, Configurator (`SegmentDefinition`/`SegmentOption` since Step 2, `ConfigurableProduct` since Step 3, `ProductSegmentAssignment` since Step 4, `SavedConfiguration` since Step 6, `QuoteMessage` since the quote-messaging feature — for `SavedConfiguration`/`QuoteMessage` the actor can be the owning end user, not an admin). The controller reads `@ActiveUser('sub') activeUserId: number`, passes it to the service method, which passes it to the provider. Note the quote-message status bumps (a message post flipping `quoteStatus`) deliberately write **no** `SavedConfiguration` audit row — the message's own `CREATE`/`'QuoteMessage'` row covers the action; only the manual admin `PATCH` audits `UPDATE`/`'SavedConfiguration'`.
