...
- Threaded `activeUserId` parameter — Users, Tags, Products, AvatarOptions, Configurator (`SegmentDefinition`/`SegmentOption` since Step 2, `ConfigurableProduct` since Step 3, `ProductSegmentAssignment` since Step 4). The controller reads `@ActiveUser('sub') activeUserId: number`, passes it to the service method, which passes it to the provider.
