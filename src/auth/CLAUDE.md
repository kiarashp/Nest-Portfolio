...
**`errorCode` on the unverified-email branch:** `SignInProvider.signIn` throws `UnauthorizedException` (401) for all three failure branches (Google-only account, unverified email, wrong password), but only the unverified-email branch passes an object payload — `new UnauthorizedException({ message, errorCode: 'EMAIL_NOT_VERIFIED' })` — instead of a plain string. Nest's `HttpException` passes an object response through largely unchanged, merging in `statusCode`/`error`, so the final body is `{ statusCode: 401, message, errorCode: 'EMAIL_NOT_VERIFIED', error: 'Unauthorized' }`. This lets the frontend distinguish "needs email verification" from "wrong credentials" without string-matching `message`. It is a targeted fix for this one ambiguity — there is no global exception filter or app-wide `errorCode` convention, so don't assume other exceptions carry one.
