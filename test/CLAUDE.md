...
The configurator suites have their own required order: delete `ConfigurableProduct` rows **before** `SegmentDefinition` rows — assignments cascade away with their product (`ProductSegmentAssignment.productId` is `onDelete: CASCADE`), but a definition cannot be deleted while any assignment still references it (`definitionId` is `onDelete: RESTRICT`). Also note `repo.delete()` ignores the soft-delete column, so it removes soft-deleted `ConfigurableProduct` fixtures too — no `withDeleted` lookup needed in cleanup. See `test/configurator/resolve.e2e-spec.ts`. `SavedConfiguration` rows never need explicit cleanup: they cascade away with their owning user (`userId` is `onDelete: CASCADE`), so running `cleanupUsers` first covers them, and their `productId` FK is `SET NULL`, so product deletion order doesn't matter for them (see `test/configurator/saved-configurations.e2e-spec.ts`).
